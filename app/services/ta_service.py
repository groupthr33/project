...
class TaService:
    ...
    def unassign_ta_to_course(self, account, course_id, section):
        courses = Course.objects.filter(course_id__iexact=course_id, section__iexact=section)
        if courses.count() == 0:
            return f"Course with ID {course_id}-{section} does not exist."
        course = courses.first()
        ...
        ta_course = TaCourse.objects.filter(course=course, assigned_ta=ta)
        if ta_course.count() == 0:
            return f"{account} is not assigned to {course_id}-{section}."

        for lab in ta_course:
            lab.delete()

        return f"{account} has been unassigned from {course_id}-{section}."

    def unassign_ta_to_lab(self, ta_user_name, course_id, course_section, lab_sections, requester):
        tas = Account.objects.filter(username__iexact=ta_user_name)
        if tas.count() == 0:
            return f"TA with user_name {ta_user_name} does not exist."
        ta = tas.first()
