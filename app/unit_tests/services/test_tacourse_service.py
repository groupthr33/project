...
from app.services.tacourse_service import TaService
...
class TestTaCourseService(TestCase):
    ...
    def test_assign_ta_happy_path(self):
        ...
        actual_response = self.tacourse_service.assign_ta(self.taName, self.course_id, self.section, self.numLabs)

        self.assertEqual(expected_response, actual_response)

    def test_course_dne(self):
        expected_response = "course CS666 dne"
        actual_response = self.tacourse_service.assign_ta(self.taName, "CS666", self.section, self.numLabs)

        self.assertEqual(expected_response, actual_response)

    def test_course_section_dne(self):
        expected_response = "section 002 dne"
        actual_response = self.tacourse_service.assign_ta(self.taName, self.course_id, "002", self.numLabs)

        self.assertEqual(expected_response, actual_response)

    def test_ta_dne(self):
        expected_response = "theta assigned to CS535-001"
        actual_response = self.tacourse_service.assign_ta("thebadta", self.course_id, self.section, self.numLabs)

        self.assertEqual(expected_response, actual_response)

    def test_ta_already_assigned(self):
        TaCourse.objects.create(remaining=0, course=self.course, assigned_ta=self.ta)
